...
class ServerNode(Printable):
    ...
    def increment_state(self, client_id, increment_distance=1):
        if client_id not in self.state:
            raise "Tried to increment a non-existant client axis"
        self.state[client_id] += increment_distance
    ...
    def find_source_of(self, change, remote_id):
        ...
        """
        cur_node = self
        while cur_node._get_client_state != change.src_client_state \
                or cur_node._get_rel_server_state != change.src_rel_server_state:
    ...
    def transform_to_tip(self, source_operation, remote_id):
        ...
        transformed_op.end.increment_state(remote_id)
    ...
    def make_ack(self, remote_id):
        ...
        return Ack(
            ...
            rel_server_state: self._get_rel_server_state(remote_id))
    ...
    def make_change(self, remote_id):
        ...
        return Change(
            ...
            src_rel_server_state: self._get_rel_server_state(remote_id),
    ...
    def _get_rel_server_state(self, client_axis):
