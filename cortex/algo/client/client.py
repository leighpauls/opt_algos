...
class Client:
    ...
    _root -- the oldest historical node that the server could send changes
    relative to, where to send changes to the server from, and this will be in
    line of incomming server changes
    _local_change_q -- Q of ClientNodes whose local_ops should be sent to the
    server
    _send_change_cb -- function to call when I want to send a change to the
    server, (Change) -> void
    _pending_ack -- True iff I'm waitin for the server to ack my last local
    change before sending the next
    """
    ...
    def apply_server_change(self, change):
        """Resolve the server change down to the tip and apply it to the local
        value
        Params:
    ...
    def apply_server_ack(self, ack):
        ...
        # I'm gaurenteed to have a straight line of history from root to (ack -
        # 1 local)
        ...
        self._root = (self._local_change_q[0]
                      if len(self._local_change_q) > 0
                      else self._tip)
        self._try_send_change()
    ...
    def _try_send_change(self):
        ...
        new_change = Change(
            src_client_state=self._root.local_state,
